...
internal class LongTextScrollerPluginShapeSource : IShapeSource
{
    ...
    void UpdateTextAndYIfNeeded()
    {
        ...
        var startLine = Math.Max(1, (int)Math.Floor(scrollLine + (lightweightTextScrollingShapeParameter.ShouldShiftHalfLine ? 0.5d : 1d)));
        var endLine = Math.Min(lineStartIndexes.Length, startLine + lineCount);
        var paragraphAlignment = lightweightTextScrollingShapeParameter.ParagraphAlignment switch
        {
    ...
    void UpdateCommandList()
    {
        ...
        _ = commandList.Close();//CommandListはEndDraw()の後に必ずClose()を呼んで閉じる必要がある
    }
